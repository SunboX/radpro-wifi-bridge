...
namespace esp_usb
{
    ...
    private:
        ...
        typedef struct vcp_driver
        {
            ...
            //  explicit type for the 'open' function (no C++20 needed)
            using open_func_t = CdcAcmDevice *(*)(uint16_t vid,
                                                  const cdc_acm_host_device_config_t *cfg,
                                                  uint8_t interface_idx);

            vcp_driver(open_func_t open_func, uint16_t _vid, std::vector<uint16_t> _pids)
                : open(open_func), vid(_vid), pids(std::move(_pids)) {}
        } vcp_driver;
